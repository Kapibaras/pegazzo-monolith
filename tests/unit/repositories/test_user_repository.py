...
class TestUserRepository:
    ...
    def test_get_all_users_by_role(self):
        ...
        # Mock
        sample_users = [User(role_id=1), User(role_id=1)]
        self.mock_query.filter.return_value.all.return_value = sample_users

        # Act
        result = self.repository.get_all_users_by_role(1)

        # Assert
        self.mock_db.query.assert_called_once_with(User)
        called_arg = self.mock_query.filter.call_args[0][0]
        assert called_arg.compare(User.role_id == 1)
        self.mock_query.filter.return_value.all.assert_called_once()
        assert result == sample_users
