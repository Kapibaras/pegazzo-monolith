...
from app.auth import RequiresAuth
...
def test_requires_checker_allows_valid_owner():
    mock_authorize = MagicMock()
    mock_authorize.jwt_required.return_value = None
    mock_authorize.get_raw_jwt.return_value = {"sub": "owner_user", "role": "propietario"}

    checker = RequiresAuth(["propietario"])
    user = checker(authorize=mock_authorize)

    assert user.username == "owner_user"
    assert user.role == "propietario"
...
def test_role_checker_valid_when_role_last_in_allowed_list():
    mock_authorize = MagicMock()
    mock_authorize.jwt_required.return_value = None
    mock_authorize.get_raw_jwt.return_value = {"sub": "employee_user", "role": "empleado"}

    checker = RequiresAuth(["propietario", "administrador", "empleado"])

    user = checker(authorize=mock_authorize)

    assert user.username == "employee_user"
...
def test_role_checker_allows_any_role_when_no_allowed_roles_defined():
    ...
    checker = RequiresAuth([])

    user = checker(authorize=mock_authorize)

    assert user.username == "any_user"
    assert user.role == "random_role"
